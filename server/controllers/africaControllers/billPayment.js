...
// 🟩 Upload Controller
export async function uploadBillPayment(req, res) {
    if (!req.file) return res.status(400).send("No file uploaded");
    try {
        await move(req.file.path, excelFilePath, { overwrite: true });
        console.log("✅ Africa Bill Payment file saved at:", excelFilePath);
...
export async function processBillPayment(req, res) {
    try {
        ...
        console.log("✅ Africa Bill Payment Excel processed.");
...
// 📥 Download Controller
export async function downloadBillPayment(req, res) {
    try {
